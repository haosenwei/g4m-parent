...
<script type="text/javascript">
	...
	$('#sysMenuDialog_aceAdd').on('click',function() {
		aceAdd("添加菜单","${server}sys/menu/toAdd?${_csrf.parameterName}=${_csrf.token}","${server}sys/menu/doAdd?${_csrf.parameterName}=${_csrf.token}","sysMenu_add_form","sysMenuDialog");
	});
	//修改
	function aceEditf(id) {
		aceEdit(id,"修改菜单","${server}sys/menu/toEdit?${_csrf.parameterName}=${_csrf.token}","${server}sys/menu/doEdit?${_csrf.parameterName}=${_csrf.token}","sysMenu_edit_form","sysMenuDialog");
	}
	//查看
	...
	//删除
	function aceDelF(id) {
		aceDel(id , 'sysMenuDialog',"${server}sys/menu/del?${_csrf.parameterName}=${_csrf.token}");
	}
	...
	$('.page-content-area').ace_ajax('loadScripts',[],function() {
		...
		initMainTable("sysMenuDialog","${server}sys/menu/search?${_csrf.parameterName}=${_csrf.token}",columnsDialog);
